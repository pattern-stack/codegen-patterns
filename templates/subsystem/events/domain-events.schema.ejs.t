 ...
 *   - `tenant_id`  — always present (nullable). Multi-tenancy is a runtime
 *                    scoping concern, not a schema-shape one: the DrizzleEventBus
 *                    and the OBS-LIST-1 read model reference `tenantId`
 *                    unconditionally, so the column must exist for every
 *                    install. Single-tenant projects simply leave it null.
 *
...
export const domainEvents = pgTable(
  'domain_events',
  {
    ...
    /** Tenant scope (nullable). Always present; null for single-tenant installs. */
