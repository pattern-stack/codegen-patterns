...
const hydrationCache = {
	dealState: new Map<string, DealState>(),
...
export async function hydrateResolverCache(): Promise<void> {
	await Promise.all([
		dealStateApi.listAll().then((rows) => {
			hydrationCache.dealState = new Map(rows.map((r) => [r.id as string, r]));
		}),
 ...
 *
 * Usage:
 *   const dealState = resolvers.dealState(other.dealStateId);
 */
export interface Resolvers {
	dealState: (id: string | null | undefined) => DealState | undefined;
...
export function createResolvers(): Resolvers {
	return {
		dealState: (id) => {
			...
			return (dealStateCollection.state.get(id) ??
				hydrationCache.dealState.get(id)) as DealState | undefined;
		},
		person: (id) => {
			if (!id) return undefined;
