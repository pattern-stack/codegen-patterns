...
/**
 * Paginated list use-case for persons (pagination-by-default).
 *
 * Composes `service.list({ where, limit, offset, sort })` + `service.count(where)`
 * into a `Page<Person>` envelope. Defaults: page 1,
 * pageSize 50 (max 200), sort `id desc` — persons declares no
...
export class ListPersonsUseCase {
  ...
  async execute(query?: ListQuery): Promise<Page<Person>> {
    ...
    // (REL-2 §2.4). A pre-rendered fragment would name `persons`,
    ...
    const known =
      (persons as unknown as Record<string, unknown>)[sortColumn] !== undefined;
