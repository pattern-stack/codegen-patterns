...
/**
 * Paginated list use-case for accounts (pagination-by-default).
 *
 * Composes `service.list({ where, limit, offset, sort })` + `service.count(where)`
 * into a `Page<Account>` envelope. Defaults: page 1,
 * pageSize 50 (max 200), sort `id desc` — accounts declares no
...
export class ListAccountsUseCase {
  ...
  async execute(query?: ListQuery): Promise<Page<Account>> {
    ...
    // (REL-2 §2.4). A pre-rendered fragment would name `accounts`,
    ...
    const known =
      (accounts as unknown as Record<string, unknown>)[sortColumn] !== undefined;
