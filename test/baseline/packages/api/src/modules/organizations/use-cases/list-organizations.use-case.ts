...
export class ListOrganizationsUseCase {
  ...
  async execute(query?: ListQuery): Promise<Page<Organization>> {
    ...
    const col = (organizations as unknown as Record<string, unknown>)[
      resolved.sortBy.replace(/_([a-z])/g, (_m: string, c: string) => c.toUpperCase())
    ];
    const orderBy: SQL =
      col === undefined
        ? sql`${desc(organizations.id)}`
        : sql`${dir(col as never)}, ${desc(organizations.id)}`;
