...
/**
 * The relation keys a opportunity read may include — the `with` surface of
 * `opportunities` in this project's graph.
 *
 ...
 */
export type OpportunityInclude = IncludeOf<'opportunities'>;

/** The row shape a given opportunity include tree resolves to, nested exactly as asked. */
export type OpportunityResult<TWith extends OpportunityInclude> =
  ResultOf<'opportunities', TWith>;

/** The default `TWith` — no include, so the plain row. */
export type OpportunityNoInclude = Record<string, never>;
 ...
 * OPTIONAL, the row's own columns required. `OpportunityResult<OpportunityInclude>`
 ...
 */
export type OpportunityApiResult = Opportunity &
  Partial<OpportunityResult<OpportunityInclude>>;

@Injectable()
export class OpportunityRepository extends BaseRepository<Opportunity, typeof opportunities, Relations> {
  ...
  // goes through RQBv2 (`db.query.opportunities`), because `baseQuery()`
  ...
  override async findById<TWith extends OpportunityInclude = OpportunityNoInclude>(
    id: string,
    opts?: { with?: TWith },
  ): Promise<OpportunityResult<TWith> | null> {
    if (opts?.with === undefined) {
      const rows = await this.baseQuery(eq(this.table['id'], id)).limit(1);
      return (rows[0] as OpportunityResult<TWith>) ?? null;
    }
    ...
    return (row as OpportunityResult<TWith> | undefined) ?? null;
  }

  override async list<TWith extends OpportunityInclude = OpportunityNoInclude>(
    query?: ListOptions & { with?: TWith },
  ): Promise<Array<OpportunityResult<TWith>>> {
    if (query?.with === undefined) {
      return (await super.list(query)) as Array<OpportunityResult<TWith>>;
    }
    if (query.orderBy !== undefined) {
      ...
      throw new Error(
        'OpportunityRepository.list: `orderBy` cannot be combined with `with` — ' +
          'pass `sort: [{ column, direction }]` instead.',
      );
    }
    const callerWhere = query.where;
    const callerSort = query.sort;
    const rows = await this.db.query.opportunities.findMany({
      where: {
        AND: [
    ...
    return rows as Array<OpportunityResult<TWith>>;
  }
  ...
  async findByOrganizationId<TWith extends OpportunityInclude = OpportunityNoInclude>(
    id: string,
    ...
    if (opts?.with === undefined) {
      ...
      return (await q) as Array<OpportunityResult<TWith>>;
    }
    const rows = await this.db.query.opportunities.findMany({
      where: {
        AND: [
  ...
  async findByOwnerId<TWith extends OpportunityInclude = OpportunityNoInclude>(
    id: string,
    opts?: { cursor?: string; limit?: number; with?: TWith },
  ): Promise<Array<OpportunityResult<TWith>>> {
    if (opts?.with === undefined) {
  ...
  async findByStateId<TWith extends OpportunityInclude = OpportunityNoInclude>(
    id: string,
    opts?: { cursor?: string; limit?: number; with?: TWith },
  ): Promise<Array<OpportunityResult<TWith>>> {
    if (opts?.with === undefined) {
