...
/**
 * Paginated list use-case for opportunities (pagination-by-default).
 *
 * Composes `service.list({ where, limit, offset, sort })` + `service.count(where)`
 * into a `Page<Opportunity>` envelope. Defaults: page 1,
 * pageSize 50 (max 200), sort `id desc` — opportunities declares no
...
export class ListOpportunitiesUseCase {
  ...
  async execute<TWith extends OpportunityInclude = OpportunityNoInclude>(
    query?: ListQuery,
    include?: TWith,
  ): Promise<Page<OpportunityResult<TWith>>> {
    ...
    // (REL-2 §2.4). A pre-rendered fragment would name `opportunities`,
    ...
    const known =
      (opportunities as unknown as Record<string, unknown>)[sortColumn] !== undefined;
