...
export class ListOpportunitiesUseCase {
  ...
  async execute(query?: ListQuery): Promise<Page<Opportunity>> {
    ...
    const col = (opportunities as unknown as Record<string, unknown>)[
      resolved.sortBy.replace(/_([a-z])/g, (_m: string, c: string) => c.toUpperCase())
    ];
    const orderBy: SQL =
      col === undefined
        ? sql`${desc(opportunities.id)}`
        : sql`${dir(col as never)}, ${desc(opportunities.id)}`;
