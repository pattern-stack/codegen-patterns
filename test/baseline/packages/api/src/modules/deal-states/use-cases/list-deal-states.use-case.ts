...
import { deal_states } from '../deal-state.entity';
import type {
  DealStateInclude,
  DealStateNoInclude,
  DealStateResult,
...
export class ListDealStatesUseCase {
  ...
  async execute<TWith extends DealStateInclude = DealStateNoInclude>(
    query?: ListQuery,
    include?: TWith,
  ): Promise<Page<DealStateResult<TWith>>> {
    ...
    // (REL-2 §2.4). A pre-rendered fragment would name `deal_states`,
    ...
    const known =
      (deal_states as unknown as Record<string, unknown>)[sortColumn] !== undefined;
