...
`sync-audit.schema.ts`, etc.) are generated by templates, not copied, so they
...
and external sync. They're auto-discovered by Claude Code from `.claude/skills/`.
...
The external-system sync engine: the `IChangeSource<T>` port (poll / CDC / webhook), `ISyncSink<T>`, `ExecuteSyncUseCase`, per-entity feature modules, the `detection:` config block, and the structured `changed_fields` audit log.

**Read [`consumer/sync.md`](consumer/sync.md)** when pulling or pushing data between your database and an external system (CRM, etc.).
