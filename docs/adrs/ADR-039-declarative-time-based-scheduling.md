...
- **BullMQ (SHIPPED — BULLMQ-2 / ADR-041, 2026-06-15)** — `BullMQEventBus` (which keeps the Postgres `domain_events` outbox, extending `DrizzleEventBus`) registers one BullMQ Job Scheduler per scheduled-event type via `upsertJobScheduler({ every: ms })`; each fired tick routes through the **same** `materializeScheduledEvent` slot-key `ON CONFLICT` insert, so the time→fact→bridge→job flow is byte-identical to the Drizzle backend. Reconcile-on-boot = upsert-desired + prune-orphans (`getJobSchedulers` → `removeJobScheduler` for orphans), closing the ENG-605 zombie-scheduler hole. Wiring lives in `BullMQEventBus.onApplicationBootstrap` (after the bridge trigger registry is populated). Under `backend: 'bullmq'` the polling `EventScheduler` setInterval loop is NOT started — exactly one scheduler source of truth per backend (ADR-041). The YAML contract is unchanged; only the materializer swaps. (The deleted Redis pub/sub backend never supported scheduling — no outbox history; replaced cleanly by the durable bullmq backend.)
