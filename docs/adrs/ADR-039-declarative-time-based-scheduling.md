...
- **BullMQ driver (SHIPPED — BULLMQ-2 / ADR-041 option #2, 2026-06-15)** — the cadence/clock can run on BullMQ while the event bus stays Drizzle (the event log is NOT moved to Redis — a Redis enqueue can't be atomic with a Postgres commit, so events stay on `pg_notify`). Selected by `events.scheduler.driver: 'bullmq'` (orthogonal to `events.backend`, default `'poll'`). A standalone `BullMqEventSchedulerLifecycle` (`runtime/subsystems/events/event-scheduler.bullmq-backend.ts`) registers one BullMQ Job Scheduler per scheduled-event type via `upsertJobScheduler({ every: ms })`; each fired tick calls `EVENT_BUS.materializeScheduledEvent(...)` — the **same** slot-key `ON CONFLICT` insert into the Drizzle outbox — so the time→fact→bridge→job flow is byte-identical to the poll driver. Reconcile-on-boot = upsert-desired + prune-orphans (`getJobSchedulers` → `removeJobScheduler`), closing the ENG-605 zombie-scheduler hole. Wiring runs in `onApplicationBootstrap` (after the bridge trigger registry is populated). When the bullmq driver is active the polling `EventScheduler` setInterval loop is NOT started — exactly one scheduler driver per process (ADR-041). `align: false` / `catchUp` are rejected under the bullmq driver (the broker scheduler is interval-anchored + stateless). The YAML `schedule:` contract is unchanged; only the driver swaps. (The deleted Redis pub/sub backend never supported scheduling — no outbox history.)
