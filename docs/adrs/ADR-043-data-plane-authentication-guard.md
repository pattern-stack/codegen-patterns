...
Use-cases that need the actor read it from the ambient context — `tryGetRequester()` — exactly as `BaseRepository.scopePredicate()` and ADR-042's `tenantPredicate()` already do. This deletes the actor-threading from the controller→use-case signature rather than re-sourcing it: the ambient context is the single, verified source, consistent with ADR-042's whole "no signature pollution" thesis. The `@ApiBearerAuth()` decorator on the controllers stays — it now matches reality (the route genuinely requires a bearer token, enforced by the guard).

> **Where the actor actually lands today (resolved during implementation).** In the current generators the `opts.actor` was consumed in exactly one place: the **event-publish metadata** of the EVT-7 emit path (`create`/`update`/`delete` use-cases + clean-arch commands). PR3 replaces `metadata: opts?.actor ? { tenantId, userId } : undefined` with a `const requester = tryGetRequester()` derived `metadata: requester ? { userId: requester.userId } : undefined`, imported via a new `tenantContextImport` local threaded through both pipelines. **`tenantId` is intentionally dropped from event metadata for now** — `RequesterContext` carries no tenant axis until ADR-042 lands; reinstating `tenantId: requester.tenantId` is a one-line follow-up gated on that ADR. The non-event use-cases simply lose the unused `_opts` parameter (`execute(dto)` / `execute(id, dto)` / `execute(id)`). Row-level `created_by` stamping was already ambient via `BaseRepository`, so it needs no use-case change.
...
- **Clean (backend)** — *revision 2026-09-19: the `clean` pipeline was deleted (ARCH-0, #677); this bullet is history. `api: false` is enforced by the backend bullet alone.* `exposeRest` / `exposeElectric` / `exposeTrpc` are AND-ed with `entity.api !== false` in `prompt.js`. The controller `to:` (gated on `exposeRest || exposeElectric`), the core module's controller import + `controllers: [...]`, and the tRPC module/router (gated on `exposeTrpc`) all already key off those flags, so flipping them off suppresses emission *and* wiring with no per-template change.
- **Backend** — *revision 2026-09-22 (REL-2, #587): the local is `apiEnabled` (the `clp` prefix was retired with the clean-lite-ps name, NAME-2 #695) and it now reads **either** form of `api:` — the boolean `api: false` or the block `api: { enabled: false }` — through one reader, so the two cannot diverge. The block is what carries `includes:` (REL-2 §5).* It gates the controller + search-controller `skip_if`, and the module template's controller/search-controller import + `controllers: [...]` array (`<% if (apiEnabled) %>`).
...
> **Note, 2026-09-20 (REL-2, #587) — the key gained an object form, and the rule held.** `api:` is now either a boolean or `{ enabled?, includes? }`, where `includes` is the HTTP include allowlist (ADR-044 §7). Two consequences for this section, neither of which weakens it:
>
> - **One reader for both forms.** `apiEnabled(def)` (`src/schema/entity-definition.schema.ts`) is what every emitter calls, so `api: false` and `api: { enabled: false }` cannot diverge. The `clpApiEnabled` local reads the same rule in the hygen half; a unit test pins the pair.
> - **No include may traverse a closed entity.** ADR-044 §7 originally allowed an allowlist to name a path through an `api: false` entity as an explicit opt-in. That clause is withdrawn (ADR-044's 2026-09-20 revision): such a path is a generation error naming both entities, so this section's "no network data plane" has no exception. Declaring `api.includes` beside `enabled: false` is the same error.
