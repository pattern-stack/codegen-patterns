...
ADR-0002 shipped `installRequesterContext` as a tested helper plus a **commented hint** in the generated `main.ts` (`init-scaffold.ts:415-416`), explicitly deferring the auto-patch. This ADR completes it, using the existing AST-patch toolkit (the OpenAPI command is the precedent):

- **At `subsystem install auth`** (and as a `project upgrade auth` command mirroring `project-upgrade-openapi.ts`), patch the consumer's `main.ts` and `app.module.ts`:
  - `ensureImport` + a `NestFactory.create`-anchored block (à la `ensureMainSwaggerBlock`, `ast-patch.ts:271`) inserts `installRequesterContext(app);` and the §4 boot-fail guard block after `const app = await NestFactory.create(...)`.
  - `ensureModuleImportEntry` ensures `AuthModule.forRoot({...})` is in `AppModule.imports` (today only the commented TODO at `templates/subsystem/auth/app-module-hook.ejs.t` exists) — and `AuthModule.forRoot` is where the `APP_GUARD` provider from §2 is registered.
- **The default scaffold `main.ts`** (`init-scaffold.ts`) replaces the commented hint with the live `installRequesterContext(app)` call + the §4 boot-fail guard, so even a hand-bootstrapped consumer who copies the default gets the closed-by-default posture, not an open one. (The scaffold must still compile without the auth subsystem present — the call is import-guarded / no-ops with a warning when `AUTH_USER_CONTEXT` is unbound, exactly as `installRequesterContext` already does, `requester-context.ts:131-139`. The boot-fail in §4 then turns that warning into a hard stop when controllers are actually exposed.)

This is the difference between "the boundary exists and you must remember to call it" and "the boundary is wired the moment auth is installed." #557 exists precisely because remembering was optional.
...
- **CI/publish coupling — each PR repairs the fixtures it breaks.** Merging to `main` auto-publishes to npm, gated by `just test-all` (unit + baseline + smoke). Two changes here break a gate and therefore cannot merge alone: the §4 boot-fail + §7 auto-wire **breaks `test-smoke`** (its no-auth scaffold now boot-fails), and the §5 header-path deletion **breaks `test-baseline`** (controller-template snapshot drift). The smoke-harness update and the baseline re-gen must ride in the *same PR* as the change that breaks them, or CI goes red on a publish branch. The new `auth.devAllowAnonymous` field (§4) is also a schema addition to `src/schema/codegen-config.schema.ts`, not just a read site.
