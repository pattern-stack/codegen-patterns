...
- Fix the three TS7053 sites. **The design's fix was wrong and was replaced — see Found #1.** What shipped: a typed
  ...
  - **Superseded by REL-0 (#603), merged into this stack.** All three assertions and every one of those
    `PgTableWithColumns<any>` occurrences are gone: `BaseRepository` is now generic over its concrete table and
    the statement builders are fed a non-generic `PgTable`. Nothing below describes current state — see
    `docs/specs/REL-0.md`.
...
| A3 | insert `.returning()` typing | `TReturning = TTable['$inferSelect']` (`insert.d.ts:84`), result `TReturning extends undefined ? QueryResult<never> : TReturning[]` (`insert.d.ts:96`). While the table type is `any` this is a union, so the result needs an assertion. `update().set().returning()` is NOT affected — `PgUpdateReturningAll` builds `TReturning` from `SelectResult<…>` (`update.d.ts:64`). **Resolved by REL-0 (#603):** the base classes now feed the builders a non-generic `PgTable`, for which the conditional resolves; the three assertions are gone. A `delete().returning()` with no projection is still a union — see REL-0 §A14. | was the 3 insert sites in `runtime/base-classes/` — none today |
| A4 | `PgTableWithColumns<T>` | `= PgTable<T> & T['columns'] & {…}` (`pg-core/table.d.ts:21`). With `T = any` the intersection collapses to `any`. With `T = TableConfig` it is **not** a supertype of a concrete table (`TableConfig['columns']` is an index signature a concrete column map does not satisfy) and string-indexing it yields `PgColumn \| undefined` under `noUncheckedIndexedAccess`, which consumer tsconfigs set. **REL-0 (#603) found the missing piece:** bare `PgTable` (declared `out`-covariant) *is* a supertype of a concrete table, which `PgTableWithColumns<TableConfig>` is not. `PgTableWithColumns` no longer appears in `runtime/base-classes/**`. | was `BaseRepository.table`; see Found #1, and REL-0 §A11–A14 for the surface that replaced it |
