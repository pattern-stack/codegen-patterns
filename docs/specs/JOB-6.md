...
- **`main-hook` injection silently no-ops against `project init`'s `main.ts` (discovered #517, pre-existing — root cause filed as issue #520).** In BOTH vendored and package mode the embedded-mode comment block does NOT land in a freshly-init'd consumer's `main.ts` (it injects fine into a clean/minimal `main.ts`, which is what the `subsystem.test.ts` unit case exercises). **Verified root cause (issue #520), correcting the earlier object-literal theory — that was wrong:** the failure is a broken `skip_if`, not the `after:` anchor. Two compounding defects:
  1. `templates/subsystem/jobs/prompt.js` returns its context object WITHOUT a `mainHookInjected` key (it returns `workerExists`, `skipSchema`, etc. but omits this one), so `skip_if: "<%= mainHookInjected %>"` interpolates an undefined local and renders the **literal string `true`** regardless of the `''` the CLI passes on `--mainHookInjected` (captured empirically: `RENDERED_SKIP_IF=[true]`).
  2. Hygen evaluates an inject-mode `skip_if` as a **regex against the whole target file**, so the rendered `true` matches any line containing the substring `true` — e.g. `project init`'s `main.ts` carries `swaggerOptions: { persistAuthorization: true }`, producing a false-positive skip. (A plain `// note: true` comment reproduces it; `{ flag: false }` injects fine — disproving the object-literal-after-anchor theory.)
  This is a `main-hook.ejs.t`/`prompt.js` bug, NOT a #517 regression — #517 wires the worker emission, which is unaffected. The package-mode smoke leg therefore asserts the worker + schema-skip, not the main-hook landing. **The bug itself is owned by issue #520** (fix: add `mainHookInjected` to the `prompt.js` return so the `''`/`'true'` arg threads through, mirroring `workerExists`); deferred from #517 since the guidance is non-functional comment text and standalone `src/worker.ts` — the operationally meaningful file — is always emitted.
