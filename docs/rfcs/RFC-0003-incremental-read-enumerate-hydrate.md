...
- **R4** — ✅ **falsifier + docs DONE; manifest decision deferred.**
  - **Falsifier (§8):** `incremental-read.spec.ts` now asserts the core guarantee — a conforming adapter emits the *identical* matching set whether the filter is pushed down (`matchesRef`, pre-hydrate) or applied at the floor (`matchesRecord`, post-hydrate); only hydration cost differs (pushdown skips dropped refs; floor hydrates all then drops).
  ...
  - **⏸ Deferred — the `filterPushdown` manifest (open design decision).** §4 wants `filterPushdown` recorded in a net-new emission manifest so consumers can see which adapters filter post-hydrate. But `filterPushdown` is **author-owned** (flipped in the emit-once scaffold), so a codegen/YAML-derived manifest can't observe the author's value. Resolving this needs a decision (recommended: promote `filterPushdown` to a `detection:` YAML field so it's codegen-known and the manifest is accurate — then re-emit it from YAML instead of a hardcoded `false`). No manifest pipeline exists today; this is net-new infra. Left for a follow-up once the decision is made.
