...
**What is mechanical vs. author-owned (corrected during E1).** The first draft called the whole sink "mechanical over the generated repository." That overclaims. The orchestrator (`execute-integration.use-case.ts:270-322`) compares `find()`'s result against `change.record` field-for-field via the differ, and both are the canonical type `T`. **`T` is defined by the adapter author** — it's whatever the `IChangeSource.listChanges` fetch body yields (the vendor's payload shape, carrying *external* join-keys like `meetingExternalId`). Codegen does **not** own `T`, so it cannot mechanically generate the `T ↔ projection/write` field mapping — that mapping is the **same author seam as the fetch body** (same author, same knowledge). What IS mechanical is the **plumbing**:

- constructor `(repo, provider)` — provider bound at construction (matches swe-brain `transcript.sink.ts`);
- the provider-match assert in `upsertByExternalId` (a mismatch is a wiring bug → throw);
- delegation to the `integrated` repo's `findByExternalIdProjected(externalId, provider)` / `integrationUpsertOne(write, provider)` / `softDeleteByExternalId(externalId, provider)` (`integrated-entity-repository.ts:83,155,179`);
- `userId` scoping as a post-filter on `find` (the repo lookup is `(provider, externalId)`-scoped);
- the `{ id, saved }` return shapes.

So the default sink is a **scaffold**: codegen emits the plumbing and a `canonical ↔ local` field-mapping seam the author fills. For the common case — FK-free entities (scalar `fields:` only, like every entity in the `integration-patterns` fixture) — the generated `<Entity>IntegrationProjection` *is* the canonical shape, so the scaffold compiles as a passthrough with no author edits. For entities carrying external FK join-keys, the scaffold leaves those as a marked author TODO. The scaffold types `T` as `<Entity>IntegrationProjection` by default (an exported `<Entity>Canonical` alias the author can widen).

> **The fully-mechanical sink is deferred to a future canonical-model RFC ("B", provisionally RFC-0004 — RFC-0003 is the disjoint incremental-read reshape).** Making codegen *own* the canonical type — emit `<Entity>IntegrationCanonical`, constrain adapters to yield it, and generate the whole mapping — requires integration entities to adopt the **join-key model** (store external FK keys as columns, resolve via downstream read-only joins, retire ingest-time FK resolution). That is a *pattern-layer* change (Integrated repo write contract + query layer), not a sink-emission change, and it carries real decisions (which refs become join-keys; where the canonical type is emitted; migrating existing resolve-based entities). It gets its own RFC, coordinated with RFC-0003 so both land in the 0.13 train. This RFC ships the scaffold; "B" upgrades it to `@generated`.

**Precondition (E1):** the default sink is emittable only for `pattern: Integrated` entities (the only family with the `integrationUpsertOne`/projection path). A non-integrated `surface:` entity → E1 **hard-errors** (clear message naming the entity + the missing `pattern: Integrated`), never emits a non-compiling sink.

Emit-once (`// <CODEGEN-SCAFFOLD-V1>`) so an author override survives regen — swe-brain's three hand-authored sinks (`TranscriptIntegrationSink` et al.) *are* such overrides and are skipped on regen.
