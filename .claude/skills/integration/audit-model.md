...
| `last_integration_at` | ts NULL | Stamped by `PostgresCursorStore.put()` alongside cursor |
| `tenant_id` | text NULL | Scaffold-time conditional (`integration.multi_tenant: true`) |
| `created_at` / `updated_at` | ts |  |

**Indexes:**
- `uq_integration_subscriptions_tuple` — unique on `(integration_id, adapter, domain, external_ref)`. Postgres treats NULL `external_ref` as distinct (intentional — NULL = "full domain" covers only one logical slot but the DB doesn't prevent duplicates there; that's a consumer modeling concern).
- `idx_integration_subscriptions_enabled_last_integration` — the scheduling query: "list enabled subscriptions ordered by staleness."

### `integration_runs`

One row per invocation of `ExecuteIntegrationUseCase.execute()`.
...
**Indexes:**
- `idx_integration_runs_subscription_started_at` — timeline: "most recent runs for this subscription"
- `idx_integration_runs_status_started_at` — stale-run sweeper: "runs that started > N minutes ago and are still running"
...
**Indexes:**
- `idx_integration_run_items_run_created_at` — timeline within a run
- `idx_integration_run_items_entity_external` — per-record history: "every integration that touched opportunity/$extId"
...
```sql
SELECT id, adapter, domain, external_ref, last_integration_at
FROM integration_subscriptions
WHERE enabled = true
  AND (last_integration_at IS NULL OR last_integration_at < now() - interval '1 hour')
ORDER BY last_integration_at ASC NULLS FIRST;
```

Uses `idx_integration_subscriptions_enabled_last_integration`.
...
Uses `idx_integration_runs_status_started_at`. Should return zero rows under
...
### "Per-record history: every integration that touched this opportunity"
...
Uses `idx_integration_run_items_entity_external`.
...
/ writes `integration_subscriptions.cursor`; `DrizzleIntegrationRunRecorder` reads /
writes `integration_runs` + `integration_run_items`. Consumers query them freely
...
`examples/integration/` is the correct placement for reference schemas if a
future consumer wants CRUD scaffolding over integration audit data.
