...
|---|---|---|
| `backend` | `JobsDomainModule.forRoot` + the worker (`JobWorkerModule.forRoot` embedded, `jobWorkerOptions` standalone) | `'drizzle'` (default), `'memory'` (tests) or `'bullmq'`. `jobWorkerBackendOptions` always states it in the worker options — the worker never falls back to `forRoot`'s `?? 'drizzle'` (JOBS-0, #656). `memory` with a standalone worker (`worker_mode: standalone`) is a config error: a separate process cannot share the in-memory store. `memory` has no job-draining worker outside tests (#658). |
...
| `worker_mode` | The jobs composer in `<generated>/subsystems.ts` | `embedded` (the default — `JobsConfigSchema`'s one `.default('embedded')`, the value the jobs-config injector writes; JOBS-1, #659) adds `JobWorkerModule.forRoot({ mode: 'embedded', … })` to `SUBSYSTEM_MODULES`; `standalone` omits it — run `src/worker.ts` (`bun src/worker.ts`) separately. Both entrypoints are always emitted; switching regenerates the barrel. The scaffold templates read no worker mode. |
