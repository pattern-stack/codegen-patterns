/**
 * ADR-033.2 — Typed provider artifacts (integration-source.providers.ts).
 *
 * Two assertions:
...
describe('integration-source.providers template (ADR-033.2)', () => {
  ...
  it('typo in consumer registry key fails tsc --noEmit (negative-case)', () => {
    ...
    try {
      ...
      writeFileSync(
        join(dir, 'opportunity-integration-source.providers.ts'),
      ...
      writeFileSync(
        ...
        [
          "import type { OpportunityProvider } from './opportunity-integration-source.providers';",
