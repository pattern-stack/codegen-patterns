/**
 * Template-emission tests for the junction integration surface (#374).
 *
 ...
 * asserts:
 *   - TIntegrationWrite / TIntegrationProjection (role + role-less)
 *   - JunctionIntegrationConfig literal with LIVE refTable handles, deduped imports
 *   - extends JunctionIntegrationRepository<E, EWrite, EProjection>
...
describe('junction integration emission — role-bearing (opportunity_contact)', () => {
  it('derives the integration write/projection locals + parent imports', async () => {
    const locals = await localsFromYaml(ROLE_YAML);
    expect(locals.leftIntegrationWriteKey).toBe('opportunityExternalId');
    expect(locals.rightIntegrationWriteKey).toBe('contactExternalId');
    expect(locals.roleColumnCamel).toBe('role');
    expect((locals.junctionIntegrationConfig as any).leftRefTable).toBe('opportunities');
    expect((locals.junctionIntegrationConfig as any).rightRefTable).toBe('contacts');
    expect(locals.integrationParentImports).toHaveLength(2);
  });

  it('emits TIntegrationWrite with both external ids + role union + userId', async () => {
    const out = render(await localsFromYaml(ROLE_YAML));
    expect(out).toContain('export interface OpportunityContactIntegrationWrite {');
  ...
  it('emits TIntegrationProjection with composite id + local FKs + role + timestamps', async () => {
    const out = render(await localsFromYaml(ROLE_YAML));
    expect(out).toContain('export interface OpportunityContactIntegrationProjection {');
  ...
  it('extends JunctionIntegrationRepository with the three params + live refTables', async () => {
    const out = render(await localsFromYaml(ROLE_YAML));
    expect(out).toContain('extends JunctionIntegrationRepository<');
    expect(out).toContain('OpportunityContactIntegrationWrite,');
    expect(out).toContain('OpportunityContactIntegrationProjection');
  ...
  it('imports the parent tables (deduped) and keeps both pairing finders', async () => {
    ...
    expect(out).toContain('integrationUpsertOne, findByExternalIdProjected, softDeleteByExternalId');
  });
});

describe('junction integration emission — role-less (opportunity_activity)', () => {
