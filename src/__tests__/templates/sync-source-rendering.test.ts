/**
 * ADR-033.1 c — Provider-keyed sync-source module emission.
 *
 * Verifies the rendered <entity>-sync-source.module.ts:
...
const RUNTIME_SYNC_DIR = resolve(import.meta.dir, '../../../runtime/subsystems/sync');
...
describe('sync-source.ejs.t (ADR-033.1 c)', () => {
  it('emits exactly two exported runtime symbols + the module class', () => {
    ...
    expect(out).toMatch(/export class OpportunitySyncSourceModule \{\}/);
  ...
  it('typechecks the rendered module against runtime sync types under strict + noUncheckedIndexedAccess', () => {
    const dir = mkdtempSync(join(tmpdir(), 'sync-source-typecheck-'));
    try {
      const syncDir = join(dir, 'sync');
      mkdirSync(syncDir, { recursive: true });
      // Vendor the runtime sync source files (excluding the drizzle backends
      ...
      for (const file of readdirSync(RUNTIME_SYNC_DIR)) {
        if (file.endsWith('.drizzle-backend.ts')) continue;
        if (file === 'sync-audit.schema.ts') continue;
        copyFileSync(join(RUNTIME_SYNC_DIR, file), join(syncDir, file));
      }
      // Trim the barrel so it doesn't re-export the drizzle backends or
      // the audit schema we omitted.
      const trimmedBarrel = readFileSync(join(syncDir, 'index.ts'), 'utf8')
        .replace(/^.*\.drizzle-backend.*$/gm, '')
        .replace(/^.*sync-audit\.schema.*$/gm, '');
      writeFileSync(join(syncDir, 'index.ts'), trimmedBarrel, 'utf8');
      ...
      // needed — this slice only imports from @shared/subsystems/sync and
      ...
      writeFileSync(
        join(dir, 'opportunity-sync-source.module.ts'),
        ...
        JSON.stringify(
          {
            compilerOptions: {
              ...
              paths: {
                ...
                '@shared/subsystems/sync': ['./sync/index.ts'],
              },
            },
            include: ['*.ts', 'sync/**/*.ts', 'nest/**/*.ts', 'domain/**/*.ts'],
          },
          null,
          2,
        ),
      ...
      const moduleErrors = output
        .split('\n')
        .filter((line) => line.includes('opportunity-sync-source.module.ts'));
