/**
 * IntegratedEntityRepository integration-surface unit tests (#374).
 *
 * Exercises the generic inbound-integration write surface — integrationUpsertOne (FK
 ...
 * softDelete branches), and the batch integrationUpsert (per-input provider, skip
...
interface AccountIntegrationWrite {
  externalId: string;
  userId: string;
  name: string;
  domain: string | null;
...
describe('IntegratedEntityRepository.integrationUpsertOne — FK resolution', () => {
  it('resolves a self-FK opportunistically and writes it into values + set', async () => {
    ...
    await repo.integrationUpsertOne(
...
describe('IntegratedEntityRepository.integrationUpsertOne — projection', () => {
  it('projects only projectionColumns (omits provider/providerMetadata)', async () => {
    ...
    const proj = await repo.integrationUpsertOne(
...
describe('IntegratedEntityRepository.integrationUpsertOne — EAV seam', () => {
...
describe('IntegratedEntityRepository.findByExternalIdProjected', () => {
...
describe('IntegratedEntityRepository.softDeleteByExternalId', () => {
...
describe('IntegratedEntityRepository.integrationUpsert (batch)', () => {
  it('returns [] for empty input without touching the db', async () => {
    const m = makeMock();
    const repo = new AccountRepository(m.db);
    expect(await repo.integrationUpsert([])).toEqual([]);
  ...
  it('skips rows missing externalId or provider', async () => {
    ...
    const out = await repo.integrationUpsert([
