/**
 * Unit tests for DrizzleIntegrationRunRecorder (SYNC-4).
 *
...
describe('DrizzleIntegrationRunRecorder — single-tenant', () => {
  let recorder: DrizzleIntegrationRunRecorder;
  ...
  beforeEach(() => {
    ...
    recorder = new DrizzleIntegrationRunRecorder(db);
  });

  describe('startRun', () => {
    it('INSERTs into integration_runs and returns the id', async () => {
  ...
  describe('recordItem', () => {
    ...
    it('INSERTs a row with valid changedFields', async () => {
      ...
      const [{ sql, params }] = captures;
      expect(sql.toLowerCase()).toContain('insert');
      expect(sql).toContain('"integration_run_items"');
  ...
  describe('completeRun', () => {
    ...
    it('UPDATEs integration_runs with terminal status + counts + duration', async () => {
...
describe('DrizzleIntegrationRunRecorder — multi-tenant', () => {
  let recorder: DrizzleIntegrationRunRecorder;
  let captures: Captured[];
  let db: DrizzleClient;
  ...
  beforeEach(() => {
    ...
    recorder = new DrizzleIntegrationRunRecorder(db, true);
  });
...
describe('DrizzleIntegrationRunRecorder.listRecent (OBS-4) — single-tenant', () => {
  let recorder: DrizzleIntegrationRunRecorder;
  let captures: Captured[];
  let db: DrizzleClient;
  ...
  it('binds subscriptionId when provided', async () => {
    ...
    const [{ sql, params }] = captures;
    expect(params).toContain('sub-1');
    // Filter lands on integration_runs.subscription_id (the FK), not the join target id.
  ...
  it('ignores tenantId when multi-tenant mode is off', async () => {
    ...
    const [{ sql, params }] = captures;
    // `tenant_id` appears in the SELECT projection (column on integration_runs),
...
describe('DrizzleIntegrationRunRecorder.listRecent (OBS-4) — multi-tenant', () => {
  let recorder: DrizzleIntegrationRunRecorder;
  let captures: Captured[];
  let db: DrizzleClient;
...
describe('DrizzleIntegrationRunRecorder — multiTenant constructor default', () => {
  it('defaults multiTenant to false when the token is not provided', async () => {
    const { db, captures } = makeCapturingDb({
      rows: [[RUN_UUID]] as unknown[][],
    });
    const recorder = new DrizzleIntegrationRunRecorder(db, undefined);
