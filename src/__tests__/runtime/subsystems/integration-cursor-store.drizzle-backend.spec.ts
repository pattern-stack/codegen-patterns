...
describe('PostgresCursorStore — single-tenant', () => {
  ...
  describe('put', () => {
    it('UPDATEs cursor, last_integration_at, and updated_at in one statement', async () => {
      ...
      expect(sql).toContain('"cursor"');
      expect(sql).toContain('"last_integration_at"');
...
describe('PostgresCursorStore.listAll (OBS-4) — single-tenant', () => {
  ...
  it('SELECTs subscription metadata ORDERed by updated_at DESC', async () => {
    ...
    // the SELECT projection, since it's a column on integration_subscriptions).
...
describe('PostgresCursorStore — multiTenant constructor default', () => {
  it('defaults multiTenant to false when the token is not provided', async () => {
    ...
    // mirrors what DI will produce when `INTEGRATION_MULTI_TENANT` is unbound.
