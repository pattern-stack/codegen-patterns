/**
 * Unit tests for ExecuteIntegrationUseCase (SYNC-5).
 *
...
import type {
  ...
  RecordItemInput,
  CompleteRunInput,
} from '../../../../runtime/subsystems/integration/integration-run-recorder.protocol';
...
class FakeSink implements IIntegrationSink<CanonicalOpp> {
...
class FakeRecorder implements IIntegrationRunRecorder {
  ...
  async startRun(input: Parameters<IIntegrationRunRecorder['startRun']>[0]): Promise<{
    id: string;
  }> {
...
function makeOrchestrator(
  ...
  sink: IIntegrationSink<CanonicalOpp>,
  recorder: IIntegrationRunRecorder,
  cursors: MemoryCursorStore,
) {
  return new ExecuteIntegrationUseCase<CanonicalOpp>(
    source,
    cursors,
...
describe('ExecuteIntegrationUseCase', () => {
  ...
  describe('created happy path', () => {
    it('upserts, records operation=created, advances cursor', async () => {
      ...
      // external_id is integration-tracking metadata (record identity), not a domain
