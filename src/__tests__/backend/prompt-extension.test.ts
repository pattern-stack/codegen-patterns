/**
 * Unit tests for backend/entity-locals.js
 */
...
describe('buildBackendLocals', () => {
  it('derives correct class names from entity name', () => {
    const locals = buildBackendLocals(contactDefinition, EMPTY_BASE_LOCALS);
  ...
  it('maps Activity pattern to ActivityEntityRepository and ActivityEntityService', () => {
    const locals = buildBackendLocals(activityDefinition, EMPTY_BASE_LOCALS);
  ...
  it('defaults to base when pattern key is absent', () => {
    const locals = buildBackendLocals(noFamilyDefinition, EMPTY_BASE_LOCALS);
  ...
  it('generates correct output paths for entity with plural name', () => {
    ...
    expect(locals.outputPaths.entity).toBe('src/modules/contacts/contact.entity.ts');
    expect(locals.outputPaths.repository).toBe('src/modules/contacts/contact.repository.ts');
    expect(locals.outputPaths.service).toBe('src/modules/contacts/contact.service.ts');
    expect(locals.outputPaths.controller).toBe('src/modules/contacts/contact.controller.ts');
    expect(locals.outputPaths.module).toBe('src/modules/contacts/contacts.module.ts');
    expect(locals.outputPaths.findByIdUseCase).toBe('src/modules/contacts/use-cases/find-contact-by-id.use-case.ts');
    expect(locals.outputPaths.listUseCase).toBe('src/modules/contacts/use-cases/list-contacts.use-case.ts');
    expect(locals.outputPaths.createDto).toBe('src/modules/contacts/dto/create-contact.dto.ts');
    expect(locals.outputPaths.updateDto).toBe('src/modules/contacts/dto/update-contact.dto.ts');
    expect(locals.outputPaths.outputDto).toBe('src/modules/contacts/dto/contact-output.dto.ts');
  });

  it('processes belongs_to relations into BelongsToRelation shape', () => {
    const locals = buildBackendLocals(contactDefinition, EMPTY_BASE_LOCALS);

    expect(locals.belongsTo).toHaveLength(2);

    const accountRel = locals.belongsTo.find((r: any) => r.relatedEntity === 'account');
    ...
    const userRel = locals.belongsTo.find((r: any) => r.relatedEntity === 'user');
  ...
  it('excludes id and behavior fields from create DTO field list', () => {
    ...
    const fieldNames = locals.createDtoFields.map((f: any) => f.name);
    ...
    // Must include entity fields (FK fields handled separately in belongsToFkFields)
  ...
  it('includes all fields including id in output DTO field list', () => {
    ...
    const fieldNames = locals.outputDtoFields.map((f: any) => f.name);
    ...
    expect(fieldNames).toContain('email');
    expect(fieldNames).toContain('title');
    // Behavior fields NOT in outputDtoFields (they come from hasTimestamps/hasSoftDelete)
    // FK fields come from belongsToFkFields
  });

  it('derives nullable correctly for fields with nullable: true', () => {
    const locals = buildBackendLocals(contactDefinition, EMPTY_BASE_LOCALS);

    const titleField = locals.createDtoFields.find((f: any) => f.name === 'title');
    ...
    const firstNameField = locals.createDtoFields.find((f: any) => f.name === 'first_name');
  ...
  it('processes declarative queries from queries block', () => {
    ...
    const locals = buildBackendLocals(withQueries, EMPTY_BASE_LOCALS);
  ...
  it('processes via-table and select queries', () => {
    ...
    const locals = buildBackendLocals(withViaQueries, EMPTY_BASE_LOCALS);
  ...
  it('produces collision-free class names for different entities sharing a query', () => {
    ...
    const accountLocals = buildBackendLocals(accountDef, EMPTY_BASE_LOCALS);
    const opportunityLocals = buildBackendLocals(opportunityDef, EMPTY_BASE_LOCALS);
  ...
  it('includes declarativeQueries output path when queries exist', () => {
    ...
    expect(locals.outputPaths.declarativeQueries).toBe(
  ...
  it('sets declarativeQueries output path to null when no queries', () => {
    ...
    expect(locals.outputPaths.declarativeQueries).toBeNull();
  });

  it('places the module under the modulesDir local (paths.modules_dir, PATH-1)', () => {
    const locals = buildBackendLocals(contactDefinition, withEntities({ modulesDir: 'app/domain' }));

    expect(locals.outputPaths.entity).toBe('app/domain/contacts/contact.entity.ts');
    expect(locals.outputPaths.service).toBe('app/domain/contacts/contact.service.ts');
  });
...
describe('buildBackendLocals — PATTERN-5 registry integration', () => {
  ...
  it('two inheritable spine bases throw at generation', () => {
    ...
    expect(() => buildBackendLocals(def, EMPTY_BASE_LOCALS)).toThrow(
  ...
  it('hasPatternConfig is true + patternConfig populated when YAML supplies a config block', () => {
    ...
    const locals = buildBackendLocals(def, EMPTY_BASE_LOCALS);
  ...
  it('package mode rewrites library base-class imports to @pattern-stack/codegen/runtime (ADR-037)', () => {
    ...
    const locals = buildBackendLocals(def, { runtimeMode: 'package' });
...
describe('impliedBehaviors fold — pattern-implied behaviors merge into emit', () => {
  ...
  it('a Integrated entity gets hasExternalIdTracking even without re-declaring the behavior', () => {
    const locals = buildBackendLocals(integratedNoExplicitBehavior, EMPTY_BASE_LOCALS);
    expect(locals.hasExternalIdTracking).toBe(true);
  });
  ...
  it('explicit external_id_tracking declaration stays a no-op (silent dedup)', () => {
    ...
    const implied = buildBackendLocals(integratedNoExplicitBehavior, EMPTY_BASE_LOCALS);
    const declared = buildBackendLocals(explicit, EMPTY_BASE_LOCALS);
    ...
    expect(declared.drizzleImports).toEqual(implied.drizzleImports);
  });
  ...
  it('a Base entity (no pattern) does NOT gain external_id_tracking', () => {
    ...
    const locals = buildBackendLocals(base, EMPTY_BASE_LOCALS);
...
describe('renderPatternConfigLiteral — idiomatic TS literal emission', () => {
  ...
  const mod = require('../../../templates/entity/new/backend/entity-locals.js') as {
    buildBackendLocals: typeof buildBackendLocals;
  };
  // The helper is exposed via the locals object; grab it by rendering a
  // throwaway entity.
  const sampleLocals = mod.buildBackendLocals(
    {
...
describe('capability composition emission (ADR-041)', () => {
  ...
  it('two or more capabilities → a composed-base file, rightmost outermost', () => {
    ...
    expect(locals.outputPaths.composedBase).toMatch(
  ...
  it('a library capability mixinImport is rewritten per runtime mode; an app alias is not', () => {
    registerCapabilities();
    const vendored = buildBackendLocals(
    ...
    const pkg = buildBackendLocals(entityWith(['CeGroup', 'CeAudited']), {
      runtimeMode: 'package',
    });
  ...
  it('an explicit `configProperty` wins, and no config block means nothing is emitted', () => {
    ...
    const locals = buildBackendLocals(entityWith(['CeNamed']), EMPTY_BASE_LOCALS);
  ...
  it('names the spine side a capability method collides with', () => {
    ...
    expect(() =>
      buildBackendLocals(entityWith(['CeRepoColliding', 'Integrated']), EMPTY_BASE_LOCALS),
  ...
  it('a capability method colliding with a spine-inherited method throws at generation', () => {
    ...
    expect(() =>
      buildBackendLocals(entityWith(['CeSpineColliding', 'Integrated']), EMPTY_BASE_LOCALS),
...
describe('roles: → belongs_to emission (CAP-2)', () => {
  ...
  it('a one-role emits its FK column, keyed by the ROLE, indexed by default', async () => {
    ...
    const host = locals.belongsTo.find((r: { field: string }) => r.field === 'host_contact_id');
    ...
    expect(
      locals.tableConstraints.map((c: { expr: string }) => c.expr),
  ...
  it('two one-roles to the same target get two distinct relation keys', async () => {
    ...
    expect(locals.belongsTo.map((r: { relationKey: string }) => r.relationKey).sort()).toEqual([
      'host',
      'organizer',
    ]);
  });
  ...
  it('declaring the FK field explicitly still controls required + index', async () => {
    ...
    expect(
      locals.processedFields.some((f: { name: string }) => f.name === 'host_contact_id'),
    ).toBe(false);
  });

  it('role nullable: wins over the FK field required: — the same precedence as a declared belongs_to', async () => {
    const roleLocals = buildBackendLocals(
      await withRoles(
    ...
    const declaredLocals = buildBackendLocals(
      {
        entity: { name: 'meeting', plural: 'meetings', table: 'meetings' },
    ...
    const role = roleLocals.belongsTo.find((r: { field: string }) => r.field === 'host_contact_id');
    const declared = declaredLocals.belongsTo.find((r: { field: string }) => r.field === 'host_contact_id');
...
describe('library capability configs (CAP-3, ADR-041.1)', () => {
  ...
  it('Communication: config generated from roles: — one-roles reuse the derived FK, many-roles the junction', async () => {
    restoreLibrary();
    const locals = buildBackendLocals(await meeting(), EMPTY_BASE_LOCALS);
  ...
  it('Actor individual → { kind: individual }, no import', () => {
    restoreLibrary();
    const locals = buildBackendLocals(account({ kind: 'individual' }), EMPTY_BASE_LOCALS);
  ...
  it('Actor without config is a generation error', () => {
    restoreLibrary();
    expect(() => buildBackendLocals(account(undefined), EMPTY_BASE_LOCALS)).toThrow(
  ...
  it('Actor group whose members: is not a has_many is a generation error', () => {
    restoreLibrary();
    expect(() =>
      buildBackendLocals(account({ kind: 'group', members: 'contacts' }), EMPTY_BASE_LOCALS),
