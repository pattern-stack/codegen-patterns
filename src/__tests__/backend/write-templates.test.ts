/**
 * Template rendering tests for backend write use-case + controller/module
...
describe('backend write templates — prompt-extension wiring', () => {
  ...
  it('exposes create/update/delete use-case output paths by default', () => {
    ...
    expect(locals.outputPaths.createUseCase).toBe(
      'src/modules/contacts/use-cases/create-contact.use-case.ts',
    );
    expect(locals.outputPaths.updateUseCase).toBe(
      'src/modules/contacts/use-cases/update-contact.use-case.ts',
    );
    expect(locals.outputPaths.deleteUseCase).toBe(
...
describe('backend write templates — use-case rendering', () => {
...
describe('backend write templates — controller rendering', () => {
...
describe('backend write templates — module rendering', () => {
...
describe('backend api:false HTTP-surface suppression (ADR-043 §6)', () => {
  it('apiEnabled defaults true and flips false when api: false', () => {
    expect(buildBackendLocals(baseEntity, withEntities()).apiEnabled).toBe(true);
    expect(
      buildBackendLocals({ ...baseEntity, api: false }, withEntities()).apiEnabled,
    ).toBe(false);
  });

  it('module omits the controller import + registration when api: false', () => {
    const locals = buildBackendLocals({ ...baseEntity, api: false }, withEntities());
    const output = render('module.ejs.t', locals);
