...
describe('backend eav_value_table — schema validation', () => {
...
describe('backend eav_value_table — prompt-extension locals', () => {
  it('exposes eavValueTable + definition entity metadata', () => {
    const locals = buildBackendLocals(valueEntity, withEntities());
  ...
  it('zeros the metadata when the flag is absent', () => {
    const locals = buildBackendLocals(entityWithoutFlag, withEntities());
...
describe('backend eav_value_table — @generated banner', () => {
  ...
  it('stamps the banner as the first line of the repository when the local is supplied', () => {
    const locals = { ...buildBackendLocals(valueEntity, withEntities()), generatedBanner: banner };
...
describe('backend eav_value_table — repository emission', () => {
...
describe('backend eav_value_table — service emission', () => {
...
describe('backend eav_value_table — module emission', () => {
...
describe('backend eav_value_table — composition with eav on owning entities', () => {
  it('handles a graph where one entity is eav_value_table + another is eav-enabled', () => {
    ...
    const valueLocals = buildBackendLocals(valueEntity, withEntities());
    ...
    const opportunityLocals = buildBackendLocals(opportunityEntity, withEntities());
    ...
    expect(opportunityLocals.outputPaths.findByIdWithFieldsUseCase).not.toBeNull();
  });
});
