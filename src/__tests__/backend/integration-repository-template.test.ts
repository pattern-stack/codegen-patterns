...
describe('buildIntegrationSurface derivation', () => {
  ...
  it('derives writeColumns from non-FK fields and conflictTarget (provider, externalId)', () => {
    const locals = buildBackendLocals(account, withEntities());
    expect(locals.integrationConfig).not.toBeNull();
    const cfg = locals.integrationConfig as any;
  ...
  it('names the FK write-key ${relationKey}ExternalId and self-FK refTable self', () => {
    const locals = buildBackendLocals(account, withEntities());
    const fk = (locals.integrationFkResolvers as any[])[0];
  ...
  it('non-self integrated FK uses target table name + adds a parent import', () => {
    const locals = buildBackendLocals(contact, withEntities());
    const fk = (locals.integrationFkResolvers as any[])[0];
    ...
    expect(locals.integrationParentTableImports).toEqual([
...
describe('integrated repository emission — self-FK (account)', () => {
  const out = render(buildBackendLocals(account, withEntities()) as Record<string, unknown>);
...
describe('integrated repository emission — non-self FK + eav (contact)', () => {
  const out = render(buildBackendLocals(contact, withEntities()) as Record<string, unknown>);
...
describe('non-Integrated repository emission', () => {
  const out = render(buildBackendLocals(widget, withEntities()) as Record<string, unknown>);
