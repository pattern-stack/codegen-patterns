/**
 * NAME-0 (#630) — a backend entity names every OTHER entity it references
...
describe('belongs_to', () => {
	it("takes the table export from the target's declared plural", () => {
		const holder = badge().belongsTo.find((r: Locals) => r.relatedEntity === 'person');
	...
	it("addresses a context:-nested target at its own folder", () => {
		const crew = badge().belongsTo.find((r: Locals) => r.relatedEntity === 'crew');
	...
	it('from a nested entity, climbs out of its context to a flat target', () => {
		...
		expect(locals.belongsTo[0].importPath).toBe('../../persons/person.entity');
	});
	...
	it('a self-reference needs no lookup', () => {
		...
		expect(locals.belongsTo[0]).toMatchObject({ isSelfFk: true, relatedTable: 'badges' });
	});
});
...
describe('has_many', () => {
	...
	it("resolves the target's import folder and checks it on disk at that folder", () => {
		...
		const before = roster(root, rels).hasMany[0];
		...
		expect(roster(root, rels).hasMany[0].targetExists).toBe(false);
		...
		expect(after.hasMany[0].targetExists).toBe(true);
	...
	it('a target with no entity YAML stays a soft skip (not wired, no error)', () => {
		...
		const [ghosts] = roster('src', rels).hasMany;
...
describe('field-level foreign_key: <table>.<column>', () => {
	it('imports from the entity whose YAML declares that table — not singularize(table)', () => {
		...
		expect(locals.fieldFkImports).toEqual([
...
describe('every FK callback is annotated (#631)', () => {
	...
	it('a table with no FK column does not import AnyPgColumn', () => {
		...
		expect(locals.hasFk).toBe(false);
