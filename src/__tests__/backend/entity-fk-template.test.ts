/**
 * Template rendering tests for backend entity FK emission (issue #34)
...
describe('entity FK emission (issue #34)', () => {
  it('emits .references() with onDelete: cascade for cascade relation', () => {
    const locals = buildBackendLocals(messageDefinitionCascade, EMPTY_BASE_LOCALS);
  ...
  it('emits .references() with onDelete: restrict for restrict relation', () => {
    const locals = buildBackendLocals(messageDefinitionRestrict, EMPTY_BASE_LOCALS);
    const output = render(locals as Record<string, unknown>);
  ...
  it('emits .references() with onDelete: set null for set_null relation', () => {
    const locals = buildBackendLocals(childDefinitionSetNull, EMPTY_BASE_LOCALS);
    const output = render(locals as Record<string, unknown>);
  ...
  it('defaults to restrict when on_delete is not specified in YAML', () => {
    const locals = buildBackendLocals(messageDefinitionNoOnDelete, EMPTY_BASE_LOCALS);
    const output = render(locals as Record<string, unknown>);
...
describe('soft-delete FK warning (issue #41)', () => {
  it('emits WARNING comment before FK column on soft-delete entity with cascade', () => {
    const locals = buildBackendLocals(messageDefinitionSoftDeleteCascade, EMPTY_BASE_LOCALS);
    const output = render(locals as Record<string, unknown>);
  ...
  it('emits warning on restrict relation with soft_delete too', () => {
    ...
    const locals = buildBackendLocals(softDeleteRestrict, EMPTY_BASE_LOCALS);
    const output = render(locals as Record<string, unknown>);
...
describe('external_id_tracking unique index emission', () => {
  it('emits uniqueIndex over (provider, external_id) when behavior is declared explicitly', () => {
    const locals = buildBackendLocals(integratedExplicitDefinition, EMPTY_BASE_LOCALS);
    const output = render(locals as Record<string, unknown>);
  ...
  it('imports uniqueIndex from drizzle-orm/pg-core', () => {
    ...
    expect(locals.drizzleImports).toContain('uniqueIndex');
  ...
  it('emits the index for a pattern: Integrated entity that does NOT re-declare the behavior', () => {
    const locals = buildBackendLocals(integratedImpliedDefinition, EMPTY_BASE_LOCALS);
    const output = render(locals as Record<string, unknown>);
  ...
  it('emits the index for a non-Integrated entity carrying the behavior directly', () => {
    const locals = buildBackendLocals(baseWithBehaviorDefinition, EMPTY_BASE_LOCALS);
    const output = render(locals as Record<string, unknown>);
  ...
  it('does NOT emit an index or uniqueIndex import without the behavior', () => {
    const locals = buildBackendLocals(baseNoBehaviorDefinition, EMPTY_BASE_LOCALS);
    const output = render(locals as Record<string, unknown>);

    expect(locals.drizzleImports).not.toContain('uniqueIndex');
...
// relationship moves that column out of processedFields into belongsTo.
...
describe('belongs_to FK inherits field required + index (#34 follow-on)', () => {
  it('emits .notNull() for a required:true field-backed FK (no rel.nullable)', () => {
    const locals = buildBackendLocals(messageDefinitionFieldBackedFk, EMPTY_BASE_LOCALS);
    const output = render(locals as Record<string, unknown>);
  ...
  it('derives nullable:false on the belongsTo entry from the field required flag', () => {
    const locals = buildBackendLocals(messageDefinitionFieldBackedFk, EMPTY_BASE_LOCALS);
    const rel = (locals.belongsTo as any[]).find((r: any) => r.field === 'conversation_id');
  ...
  it('emits a <table>_<col>_idx index for the field-backed FK', () => {
    ...
    expect(locals.drizzleImports).toContain('index');
  ...
  it('does NOT emit the FK column twice (it stays out of processedFields)', () => {
    const locals = buildBackendLocals(messageDefinitionFieldBackedFk, EMPTY_BASE_LOCALS);
    const output = render(locals as Record<string, unknown>);
  ...
  it('an explicit rel.nullable:true overrides the field required flag (no .notNull())', () => {
    ...
    const locals = buildBackendLocals(override, EMPTY_BASE_LOCALS);
    const output = render(locals as Record<string, unknown>);
  ...
  it('emits the index for a nullable indexed FK (index independent of nullability)', () => {
    ...
    const locals = buildBackendLocals(nullableIndexed, EMPTY_BASE_LOCALS);
    const output = render(locals as Record<string, unknown>);
  ...
  it('does NOT emit an index when the field-backed FK has no index:true', () => {
    ...
    const locals = buildBackendLocals(noIndex, EMPTY_BASE_LOCALS);
    const output = render(locals as Record<string, unknown>);

    // required → notNull, but no index
    expect(output).toContain("conversationId: uuid('conversation_id').notNull().references(");
    expect(output).not.toContain('_idx');
    expect(locals.drizzleImports).not.toContain('index');
  });
});

describe('prompt-extension processBelongsTo on_delete propagation', () => {
  it('includes onDelete in belongsTo entries', () => {
    const locals = buildBackendLocals(messageDefinitionCascade, EMPTY_BASE_LOCALS);
    const rel = (locals.belongsTo as any[]).find((r: any) => r.field === 'conversation_id');
  ...
  it('maps set_null to Drizzle set null', () => {
    const locals = buildBackendLocals(childDefinitionSetNull, EMPTY_BASE_LOCALS);
    const rel = (locals.belongsTo as any[]).find((r: any) => r.field === 'post_id');
