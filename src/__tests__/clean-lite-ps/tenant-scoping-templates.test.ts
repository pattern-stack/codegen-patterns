/**
 * TEN-1 (#585) — what `tenant_scoped: true` emits (ADR-042 §5, clean-lite-ps).
 *
 ...
 * bare single-column unique (TEN-1 §M3: `clean-lite-ps` emits none at all, so
...
describe('tenant_scoped — uniqueness is per tenant', () => {
  ...
  it('emits NO bare single-column unique for a `unique: true` field (§M3)', () => {
    ...
    const out = renderEntity(localsFor(def));
    // clean-lite-ps emits no single-column uniqueness at all, so there is
