...
describe('buildTypesContent — empty case', () => {
	test('emits AppDomainEvent = never, EventOfType degraded to DomainEvent base', () => {
		...
		// With no registered events, EventOfType/PayloadOfType degrade to the
		// base DomainEvent shape (not `never`) so generic event-handling code
		// (e.g. the bridge EventFlowService) still typechecks. See the empty-case
		// branch in buildTypesContent.
