...
const COMPOSERS: Partial<Record<SubsystemName, Composer>> = {
	...
	// block's `reporters:` when present, else emit a bare `forRoot()`. Ordered
	// LAST in `COMPOSABLE_ORDER` so it registers after the read ports it
	// composes via `@Optional()` DI.
