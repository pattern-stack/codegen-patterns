...
import { listEntityYamls, preflightEntityTargets, renderEntityTargets } from '../shared/entity-render.js';
import {
...
import { emitProviderModules } from '../shared/provider-module-generator.js';
import {
	isSemanticEnabled,
...
export class EntityNewCommand extends Command {
	...
	static usage = Command.Usage({
		...
		details: `
			Generates the backend backend module (entity, repository, service, controller, module, DTOs, use-cases) for the named entity (or all entities with \`--all\`), then runs the post-generation codegen steps that share this entrypoint:
	...
	async execute(): Promise<number> {
		...
		// Pre-flight — the per-target half shared with `junction new` (JUNC-0).
		const {
			validated,
			invalid,
			runRejections,
			...
			providerSet,
			tsAliases,
		} = await preflightEntityTargets(ctx, targets);
		const { jobsDir, providers: providersDir } = projectLayout(ctx.cwd, ctx.config);
		...
		const rendered = renderEntityTargets(ctx, validated, {
			continueOnError: this.continueOnError,
		});
		const succeeded = rendered.succeeded;
		const failed: RejectionEntry[] = [...invalid.map(rejectionEntry), ...rendered.failed];
