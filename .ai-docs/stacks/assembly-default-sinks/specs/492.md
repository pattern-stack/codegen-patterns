...
- **#505 merge timing.** RESOLVED: #505 merged at `510d847` before this branch was picked up. Rebase path taken.
- **Does `just bump minor` version-lock the surface/runtime workspace packages, or only root?** RESOLVED: root `package.json` only (justfile:219-227 confirmed). Surface packages (`packages/codegen-crm`, etc.) carry independent 0.2.x versions and are NOT bumped by the script.
- **Should the #485 correction be a comment + body edit, or comment only?** RESOLVED: comment only (non-destructive, durable; epic body not churned).
...
**Pre-flight:** #505 merged at `510d847` on origin/main. Branch rebased off post-#505 main. Snapshot provenance fix already in, deprecated shim already deleted. No delta absorption needed — pure rebase path.

**Delta 1:** Added `THREAD` fixture + `"E4 · sink knobs through emitAdapters"` describe to `assembly-contract.test.ts`. Four assertions: noop body + no repo delegation, excluded field absent from write mapping, excluded field retained in find view, no `as` cast. Suite: 43 → 48 tests; test-integration-emit: 52 → 56 pass.

**Delta 2:** `just bump minor` → 0.23.0 (root `package.json` only; surface packages have independent 0.2.x versioning, not locked to root).

**Snapshot review verdict:** Clean. Two-file shape confirmed (`meeting.sink.generated.ts` + `meeting.sink.ts`; same for crm entities). No `/tmp/sink-trilemma` provenance artifacts (removed by #505). No accumulated cruft found.

**test-all recorded results (2026-06-06):**

| Suite | Result | Count |
|---|---|---|
| test-unit | PASS | 2912 pass, 3 skip, 0 fail (183 files) |
| test-baseline | PASS | all ✅ (zero diff) |
| test-integration-emit | PASS | 56 pass, 0 fail (2 files) |
| test-smoke | PASS | scaffold + tsc + openapi clean |
| test-smoke-subsystems | PASS | boot verify OK |
| test-smoke-relationship | PASS | 6 schemas, 6 paths |
| test-smoke-junction | PASS | boot OK |
| test-smoke-junction-cross-domain | PASS | boot OK |
| test-junction | PASS | 10 pass, 0 fail (2 files) |
| test-smoke-integration | PASS | 62 .ts files, tsc OK |

**Open questions resolved:**
- #505 merge timing → merged before #492 branched; rebase path taken.
- `just bump` surface scope → root only; confirmed surface packages are independently versioned.
- #485 correction → comment only (non-destructive, durable).
