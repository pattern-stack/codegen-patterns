...
## Post-implementation correction — 2026-06-06 (#488 Gate 2.5 round 2)

**The active `record.<writeKey>` mapping accepted in §2 was unsound under the
projection-default canonical.**

The acceptance criteria for §2 stated that FK external keys should emit as
`<writeKey>: record.<writeKey>,` — i.e., a live mapping that copies the external
key out of the incoming canonical record. This was correct in isolation but
broken in the broader context: the projection-default canonical
(`<E>IntegrationProjection`) does **not** carry external-key members (e.g.
`accountExternalId`). It carries only local FK columns (e.g. `accountId`).
Emitting `accountExternalId: record.accountExternalId,` therefore produces
TS2339 the moment any entity has a `belongs_to` relationship — the member
simply does not exist on the type. This was caught by the `lead` fixture
(added in #488) whose `tsc --noEmit` gate failed on CI.

**Corrected emission (shipped in #488, `fix(sink-emitter)` commit):**

```typescript
// SEAM (FK external key — null until you widen LeadIntegrationProjection to carry `accountExternalId`):
// Replace null with record.accountExternalId after widening. Write-safe: repo skips null FKs.
accountExternalId: null,
```

Rationale:
- `null` is write-safe: `integrated-entity-repository.ts:118-120` has an
  `!== null` guard — null FK write keys are skipped and never clobber an
  existing link.
- The SEAM comment routes the author to widen the canonical and then replace
  `null` with `record.<writeKey>`. The compiler enforces this: as long as the
  canonical doesn't carry the external-key member, `record.<writeKey>` would
  be a TS2339 error, so the null stays until the author opts in.
- Mechanical FK sourcing (reverse-resolving local FK column → external key at
  read time so the projection can carry it) is **out of scope** — tracked in
  RFC-0004 (#489).

**Unit and integration-emit tests updated** in the same commit: all FK describe
blocks now assert `<writeKey>: null,` and a SEAM comment; the `codeOnly()`
helper strips `//` comment lines so `not.toContain("accountExternalId: record.")`
is not tripped by the instructional comment that legitimately mentions the
`record.` path. The `assembly-contract.test.ts` contact-sink FK assertion was
updated to match.

**Snapshot re-baselined** — `lead.sink.ts` in the snapshot now shows
`accountExternalId: null,` in the write block.
